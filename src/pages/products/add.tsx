...
const AddProduct = () => {
  ...
  useEffect(() => {
    const fetchCategories = async () => {
      ...
      const res = await handleGetProductCategories(null);
      if (res.status) {
  ...
  useEffect(() => {
    const fetchBrands = async () => {
      ...
      const res = await handleGetProductBrand(null);
      if (res.status) {
  ...
  async function handleSubmit(event: React.FormEvent): Promise<void> {
    ...
    const res = await handleCreateProduct(model, null);
    ...
    <PageContainer title="Add Product" description="Add your product on Gidella">
        <Grid container spacing={3} justifyContent="center">
            <Grid item xs={12} sm={12} lg={8} xl={8} justifyContent="center" alignItems="center">
                <DashboardCard title="Add Product" action={
                    <Button color="primary" component={Link} href="/products/list" variant="contained" size="medium">   
                        Back
                    </Button>
                    }>
